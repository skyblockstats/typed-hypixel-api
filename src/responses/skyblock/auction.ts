...
export interface SkyBlockAuctionsResponse {
	success: true
	auctions: {
		/** The ObjectID of the auction. You probably want the `uuid` instead of this. */
		_id: string
		...
		profile_id: string
		/** The UUIDs of the people in the same co-op as the creator of the auction, including the creator of the auction. */
		coop: string[]
		...
		item_lore: string
		/** The name of the item combined with the name of the Minecraft item. For example: `Itchy Piggy Bank Skull Item`. */
		extra: string
		/**
		 * The category of the item. These are the same categories that are used to sort in the auction house GUI.
		*/
		category: 'weapon' | 'accessories' | 'misc' | 'blocks' | 'consumables' | 'armor'
		/** The rarity of the item. */
		tier: ITEM_RARITY
		/** What the initial starting price set by the creator was. This is also used as the price for BIN auctions. */
		starting_bid: number
